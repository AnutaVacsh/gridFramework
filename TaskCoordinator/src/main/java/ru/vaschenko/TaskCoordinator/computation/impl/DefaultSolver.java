...
@Slf4j
@Service
public class DefaultSolver implements Solver<List<ResultLatinSquare>, List<List<Character>>, SubTask> {

    @Override
    public List<ResultLatinSquare> solve(List<List<Character>> restoreMatrix, SubTask subTask) {
        ...
        for (BigInteger num = BigInteger.ZERO; num.compareTo(totalVariants) < 0; num = num.add(BigInteger.ONE)) {
